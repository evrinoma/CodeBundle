...
//region SECTION: Public
    ...
    {
        $decoratorQuery = $container->getParameter('evrinoma.contractor.decorates.code.query');
        if ($decoratorQuery) {
        ...
        $decoratorCommand = $container->getParameter('evrinoma.contractor.decorates.code.command');
        if ($decoratorCommand) {
        ...
        $decoratorQuery = $container->getParameter('evrinoma.contractor.decorates.bunch.query');
        if ($decoratorQuery) {
            $queryMediator = $container->getDefinition($decoratorQuery);
            $repository    = $container->getDefinition('evrinoma.'.EvrinomaCodeBundle::CODE_BUNDLE.'.bunch.repository');
            $repository->setArgument(2, $queryMediator);
        }
        $decoratorCommand = $container->getParameter('evrinoma.contractor.decorates.bunch.command');
        if ($decoratorCommand) {
            $commandMediator = $container->getDefinition($decoratorCommand);
